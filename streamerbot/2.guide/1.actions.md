...
Action queues are a way to organise your actions to be run in order. Let's start from the beginning to make sure to understand how they work.
...
In here you will find all the queues that were created. You will also see the `Pending Count` of actions waiting to be executed for each queue. The `Completed Count` of actions completed and if a queue is `Paused` or not.
...
This is 1 context option that will show the current option you can use.

If the queue is "Paused" you can then select `Resume` to restart a queue
If the queue is currently "Active" you can select `Pause` to pause the queue so events that are in that queue are not triggered anymore.

- Available if you <Kbd>Right-Click</kbd> on a specific queue

##### Blocking

This is a "Toggle" option. If it's selected there will be a check mark appearing beside "Blocking"

- Available if you <Kbd>Right-Click</kbd> on a specific queue

##### Clear

Will clear the selected queue of any pending actions.
